          ...
          <div class="window">
            <div class="window-rounded">
              ...
              <div class="right-modal">
                ...
                <div class="toggle" id="toggle-far">&#176;F</div>
                <div class="toggle two active" id="toggle-cel">&#176;C</div>
              </div>
            </div>
          </div>
        </div>
...
<script>
  ...
  const outerRimElement = document.querySelector('.outer-rim');
  var angle = 0;

  const tempElement = document.querySelector('.temp');
  const labelElement = document.querySelector('.right-modal .label');
  const mountainsElement = document.querySelector('#mountains');
  const sixths = Array.from(document.querySelectorAll('.sixths'));
  const sceneClasses = ['sunset', 'moon', 'clouds', 'storm', 'snow'];
  const sceneFocusIndexMap = {
    sun: 0,
    sunset: 1,
    moon: 2,
    clouds: 3,
  ...
  const fallbackForecasts = [
    { tempF: 34, scene: 'sun', label: 'Now' },
    { tempF: 27, scene: 'sunset', label: '+3h' },
    { tempF: 14, scene: 'moon', label: '+6h' },
    { tempF: 16, scene: 'clouds', label: '+9h' },
    { tempF: 8, scene: 'storm', label: '+12h' },
    { tempF: -4, scene: 'snow', label: '+15h' }
  ];
  let forecastSlots = fallbackForecasts;
  let locationName = 'Weather';
  let selectedForecastIndex = 0;
  let realtimeTimerId = null;
  let randomIconTimerId = null;
  const REALTIME_INTERVAL_MS = 30000; // 30 seconds refresh for real-time updates
  const RANDOM_ICON_INTERVAL_MS = 15000;
  const randomIconButtonSelectors = ['.left-btn-3', '.left-btn-2', '.left-btn-1'];

  function formatLabel(slot) {
    const suffix = slot && slot.label ? ' - ' + slot.label : '';
    return locationName + suffix;
  }

  function setLabel(slot) {
    labelElement.textContent = formatLabel(slot);
  }

  function setUnitFocus(unitId, shouldConvert) {
    const targetToggle = document.getElementById(unitId);
    if (!targetToggle) {
      return;
    }

    const previousActiveToggle = document.querySelector('.toggle.active');
    const previousUnitId = previousActiveToggle ? previousActiveToggle.id : '';
    const tempText = tempElement.textContent || '';
    const tempIsCelsius = tempText.includes('C');
    const tempIsFahrenheit = tempText.includes('F');
    ...
    if (unitId === 'toggle-cel') {
      if (tempIsCelsius && !tempIsFahrenheit) {
        return;
      }
      const celsius = Math.round((currentValue - 32) * 5 / 9);
      tempElement.textContent = celsius + DEGREE + 'C';
      return;
    }

    if (tempIsFahrenheit && !tempIsCelsius) {
      return;
    }
    const fahrenheit = Math.round(currentValue * 9 / 5 + 32);
    tempElement.textContent = fahrenheit + DEGREE + 'F';
  }
  ...
  function startRealtimeUpdates() {
    ...
    realtimeTimerId = window.setInterval(function() {
      if (!outerRimElement.classList.contains('power-on')) {
        return;
      }
  ...
  function triggerRandomIconEffect() {
    const buttonElements = randomIconButtonSelectors
    ...
    const availableButtons = buttonElements.filter(function(buttonElement) {
      return !buttonElement.classList.contains('active');
    });

    const pool = availableButtons.length > 0 ? availableButtons : buttonElements;
    const randomIndex = Math.floor(Math.random() * pool.length);
    const randomButton = pool[randomIndex];

    if (typeof randomButton.click === 'function') {
      randomButton.click();
      return;
    }
  ...
  function startRandomIconUpdates() {
    if (randomIconTimerId !== null) {
      return;
    }

    randomIconTimerId = window.setInterval(function() {
      if (!outerRimElement.classList.contains('power-on')) {
        return;
      }

      triggerRandomIconEffect();
    }, RANDOM_ICON_INTERVAL_MS);
  }

  function stopRandomIconUpdates() {
    if (randomIconTimerId === null) {
      return;
    }

    window.clearInterval(randomIconTimerId);
    randomIconTimerId = null;
  }

  function syncRealtimeWithPowerState() {
    if (outerRimElement.classList.contains('power-on')) {
      setUnitFocus('toggle-cel', true);
      hydrateForecasts();
      startRealtimeUpdates();
      startRandomIconUpdates();
      return;
    }

    stopRealtimeUpdates();
    stopRandomIconUpdates();
  }

  function getSceneFocusIndex(scene) {
    return Object.prototype.hasOwnProperty.call(sceneFocusIndexMap, scene) ? sceneFocusIndexMap[scene] : 0;
  }

  function syncActiveForecastIndex(nextIndex) {
    const normalizedIndex = Math.max(0, Math.min(nextIndex, sixths.length - 1));

    sixths.forEach(function(sixth, sixthIndex) {
      sixth.classList.toggle('active', sixthIndex === normalizedIndex);
    });

    selectedForecastIndex = normalizedIndex;
    angle = normalizedIndex * 60;
  ...
  function normalizeScene(scene) {
    return sceneClasses.includes(scene) ? scene : 'sun';
  }

  function applyScene(scene) {
    const normalizedScene = normalizeScene(scene);
    mountainsElement.classList.remove(...sceneClasses);
    if (normalizedScene !== 'sun') {
      mountainsElement.classList.add(normalizedScene);
    }
  }

  function resolveScene(weatherCode, isDay) {
    ...
    if ([45, 48].includes(weatherCode) || weatherCode >= 1) {
      return isDay ? 'clouds' : 'moon';
    }

    return isDay ? 'sun' : 'moon';
  }

  function buildSlot(tempCelsius, weatherCode, isDay, label) {
    return {
      tempF: celsiusToFahrenheit(tempCelsius),
      scene: resolveScene(weatherCode, isDay),
      label: label
    };
  }

  function updateDashboard(slot, shouldAnimate) {
    if (!slot) {
      return;
    }

    setLabel(slot);
    applyScene(slot.scene);

    if (shouldAnimate) {
      changeTemp(tempElement, slot.tempF);
      return;
    }

    currentTempF = slot.tempF;
    const activeUnit = document.querySelector('.toggle.active');
    if (activeUnit && activeUnit.id === 'toggle-cel') {
      tempElement.textContent = Math.round((slot.tempF - 32) * 5 / 9) + DEGREE + 'C';
    } else {
      tempElement.textContent = slot.tempF + DEGREE + 'F';
    }
  }

  function makeRelativeHourLabel(offsetHours) {
    return offsetHours === 0 ? 'Now' : '+' + offsetHours + 'h';
  }

  function applyNightToggleByLocalTime(slots) {
  ...
  function buildForecastSlotsFromApi(weatherData) {
    const now = new Date();
    const timeValues = weatherData.hourly.time;
    const temperatureValues = weatherData.hourly.temperature_2m;
    const weatherCodes = weatherData.hourly.weather_code;
    const isDayValues = weatherData.hourly.is_day;

    let startIndex = timeValues.findIndex(function(timeValue) {
      return new Date(timeValue) >= now;
    });
    ...
    for (let offset = 0; offset < 6; offset += 1) {
      const hourOffset = offset * 3;
      const targetIndex = Math.min(startIndex + hourOffset, timeValues.length - 1);
      slots.push(buildSlot(
        temperatureValues[targetIndex],
        weatherCodes[targetIndex],
        Boolean(isDayValues[targetIndex]),
        makeRelativeHourLabel(hourOffset)
      ));
    }
  ...
  async function fetchLocation() {
    try {
      ...
      console.warn('Browser geolocation unavailable', geolocationError);
    }

    try {
      const data = await fetchJson('https://ipapi.co/json/', 'Location lookup failed');
      ...
      console.warn('ipapi.co lookup failed', ipApiError);
    }

    try {
      const data = await fetchJson('https://ipwho.is/', 'Location lookup failed');
      if (data.success) {
        return data;
      }
    } catch (ipWhoError) {
      console.warn('ipwho.is lookup failed', ipWhoError);
    }
  ...
  async function fetchAreaDetails(latitude, longitude) {
    const params = new URLSearchParams({
      format: 'jsonv2',
      lat: latitude,
      lon: longitude,
    ...
    const data = await fetchJson('https://nominatim.openstreetmap.org/reverse?' + params.toString(), 'Area lookup failed');
    const address = data.address || {};

    return {
      district: address.city_district || address.suburb || address.borough || address.quarter || address.neighbourhood || '',
      province: address.city || address.state_district || address.state || address.county || ''
    };
  }

  function buildLocationName(location, areaDetails) {
    const district = areaDetails && areaDetails.district ? areaDetails.district : '';
    const province = areaDetails && areaDetails.province ? areaDetails.province : '';
    const detailedLocation = [district, province].filter(Boolean);

    if (detailedLocation.length > 0) {
      return detailedLocation.join(', ');
    }

    return [location.city, location.region].filter(Boolean).join(', ') || location.country || 'Weather';
  }

  async function hydrateForecasts() {
    ...
    try {
      ...
      try {
        areaDetails = await fetchAreaDetails(location.latitude, location.longitude);
      } catch (areaError) {
        console.warn('Area lookup failed', areaError);
      }

      locationName = buildLocationName(location, areaDetails);

      const weatherData = await fetchForecast(location.latitude, location.longitude);
      forecastSlots = buildForecastSlotsFromApi(weatherData);
      forecastSlots = applyNightToggleByLocalTime(forecastSlots);
      syncActiveForecastIndex(getSceneFocusIndex(forecastSlots[0].scene));
      updateDashboard(forecastSlots[0], false);
    } catch (error) {
      console.error(error);
      locationName = 'Offline forecast';
      forecastSlots = fallbackForecasts;
      forecastSlots = applyNightToggleByLocalTime(forecastSlots);
      syncActiveForecastIndex(getSceneFocusIndex(forecastSlots[0].scene));
      updateDashboard(forecastSlots[0], false);
    }
  ...
  function changeTemp(element, newTemp) {
    let unit = element.innerHTML.includes("F") ? DEGREE + "F" : DEGREE + "C";
    let currentTemp = unit === DEGREE + "F" ? currentTempF : Math.round((currentTempF - 32) * 5 / 9);
    let finalTemp = unit === DEGREE + "F" ? newTemp : Math.round((newTemp - 32) * 5 / 9);
  ...
  window.onload = function() {
    setUnitFocus('toggle-cel', true);
    hydrateForecasts();
    syncRealtimeWithPowerState();

    const powerStateObserver = new MutationObserver(function() {
      syncRealtimeWithPowerState();
    });
    powerStateObserver.observe(outerRimElement, {
      attributes: true,
      attributeFilter: ['class']
    });

    powerButton.addEventListener('click', function() {
      // Wait for the inline click handler to toggle power-on first.
      requestAnimationFrame(syncRealtimeWithPowerState);
    });

    document.querySelector('#rot-icons').addEventListener('click', () => {
      const nextIndex = (selectedForecastIndex + 1) % sixths.length;
      syncActiveForecastIndex(nextIndex);
      updateDashboard(forecastSlots[selectedForecastIndex] || forecastSlots[0], true);
